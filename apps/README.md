# apps/

小工具集合。每个子目录是一个独立的 HTML+JS 应用，通过本地 HTTP 服务或 GitHub Pages 访问。

## 工具列表

- **`swarm_calculator/`** — 虫群伤害计算器：三种攻击者预设（花衣蝶/陨星虫/女王蜂5层），输入威力/连击/中毒奉献层数，支持预定义目标库管理、手动输入、6只队伍批量计算，含中毒DoT多回合斩杀预测** — 伤害计算器：5 个模块（攻击方/防御方/技能/全局状态/结果），支持精灵名自动填入种族值、按显示威力或按技能名称两种模式、属性差技能（闪击/鸣沙陷阱）阶梯、隐藏乘区（顺风/破空/风起）、防御技能减伤、中毒/灼烧/冰冻 DoT 与冻结斩杀
- **`transmission_calculator/`** — 传动计算器：模拟机械系精灵的技能位置变化，支持多精灵卡片，按回合推演每只精灵 4 个槽位的技能流转，附带"对战推断"模块（根据观察到的对手 1 号位技能反推初始排列）
- **`replay_recorder/`** — 对战回放记录：边看高分局视频边记录每回合双方动作和状态变化（血量/能量/buff/dot/印记/永久效果），数据存 localStorage，可导出 JSON 离线分析

---

## 本地调试

在项目根目录运行：

```bash
./apps/serve.sh          # 默认 8000 端口
./apps/serve.sh 8080     # 指定端口
```

然后在浏览器打开：`http://localhost:8000/apps/damage_calculator/`

脚本本质是 `python3 -m http.server`，无任何依赖。改了代码或 `data/raw/*.json` 后刷新浏览器即可生效。

**为什么不能直接双击打开 HTML？** 浏览器限制 `file://` 协议下的 `fetch()` 请求，无法读本地 JSON。

---

## GitHub Pages 部署

仓库 Settings → Pages → 选 `main` 分支 `/` 目录即可。访问路径：

```
https://<user>.github.io/<repo>/apps/damage_calculator/
```

部署后工具会通过相对路径 `../../data/raw/pokemon.json` 加载最新数据，无需重新打包。

---

## 设计原则

- **单文件**：每个工具一个 `index.html`，便于分发
- **零依赖**：不引用外部 CDN
- **数据共享**：所有工具通过 `fetch('../../data/raw/*.json')` 读取项目数据，一次维护多处复用
- **响应式**：移动端和桌面端都能用
