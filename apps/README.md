...
- **`transmission_calculator/`** — 传动计算器：模拟机械系精灵的技能位置变化，支持多精灵卡片，按回合推演每只精灵 4 个槽位的技能流转，附带"对战推断"模块（根据观察到的对手 1 号位技能反推初始排列）
- **`replay_recorder/`** — 对战回放记录：边看高分局视频边记录每回合双方动作和状态变化（血量/能量/buff/dot/印记/永久效果），数据存 localStorage，可导出 JSON 离线分析
